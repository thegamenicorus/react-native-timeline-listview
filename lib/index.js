...
export default class Timeline extends Component {
  ...
  _renderEvent(rowData, sectionID, rowID){
    ...
    var opStyle = null
    const { eventStyle, eventContainerStyle } = this.props;
    switch(this.props.columnFormat){
      case 'single-column-left':
        opStyle = {
    ...
    return (
      <View style={[styles.details, opStyle, eventContainerStyle]} onLayout={(evt)=> { if(!this.state.x && !this.state.width){var {x,width} = evt.nativeEvent.layout;this.setState({x, width})}}}>
        ...
        >
          <View style={[{paddingTop:10, paddingBottom:10}, eventContainerStyle]}>
  ...
  _renderDetail(rowData, sectionID, rowID) {
    ...
    if(rowData.description)
      title = (
        <View>
          {rowData.title && <Text style={[styles.title, this.props.titleStyle]}>{rowData.title}</Text>}
