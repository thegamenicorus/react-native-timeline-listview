...
export default class Timeline extends Component {
  ...
  _renderRow(rowData, sectionID, rowID) {
    ...
    let content = (
      <View style={[styles.rowContainer, this.props.rowContainerStyle]}>
        ...
        <View style={[styles.details, {borderColor: lineColor, borderLeftWidth: lineWidth}]}>
    ...
    if(this.onTimelinePress)
      return (
        <TouchableOpacity key={rowID} onPress={()=>this.onTimelinePress(rowData)}>
            {content}
        </TouchableOpacity>
      )
    else
  ...
  _renderCircle(rowData){
      ...
      let circleStyle = {
        ...
        left: ((circleSize/2)*-1) - ((lineWidth-1)/2),
        position: 'absolute'
      };
      ...
      switch(this.props.innerCircle){        
        case 'icon':
          ...
          innerCircle = (<Image source={iconSource} style={iconStyle} />)
        break;
        case 'dot':
          let dotStyle = {
            height: circleSize / 2,
